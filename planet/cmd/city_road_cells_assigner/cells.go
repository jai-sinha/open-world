...
func processRegion(
	...
	boundaries, citiesByStripe, err := loadCityBoundaries(cities, citiesDir, cellSize, stripeWidth)
	if err != nil {
	...
	response, err := assignRegionCells(
		...
		cellSize, stripeWidth, boundaries, citiesByStripe, started,
	)
	if err != nil {
		return response, err
	}
...
func loadCityBoundaries(
	cities []*cityRecord,
	citiesDir string,
	cellSize int,
	stripeWidth int,
) ([]cityBoundary, map[int][]int, error) {
	boundaries := make([]cityBoundary, 0, len(cities))
	...
	for _, city := range cities {
		...
		minStripe := stripeIDForCell(bounds.MinX, stripeWidth)
		maxStripe := stripeIDForCell(bounds.MaxX, stripeWidth)
...
func assignRegionCells(
	...
	regionTmpDir, err := os.MkdirTemp(outputDir, "region-"+regionName+"-")
	if err != nil {
	...
	partition, err := partitionRoadSegmentsToStripes(
		sourcePbf, highwayValues, cellSize, segmentsDir, stripeWidth,
	)
	if err != nil {
		return response, err
	}
	...
	results, err := processStripes(
		stripeIDs, partition.StripePaths, citiesByStripe,
		boundaries, cellSize, stripeWidth, stripeResultsDir,
	)
	if err != nil {
		return response, err
	}
	...
	for cityIndex, boundary := range boundaries {
		...
		if err := initializeCityRoadCellsBinary(tempPath, cellSize); err != nil {
			return response, err
		}
	...
	for cityIndex, boundary := range boundaries {
		...
		finalPath := filepath.Join(outputDir, filepath.FromSlash(boundary.RoadCellsPath))
	...
	for i := 0; i < workerCount; i++ {
		...
		go func() {
			...
			for stripeID := range jobs {
				out <- processStripe(
					stripeID, stripePaths[stripeID], citiesByStripe[stripeID],
					boundaries, cellSize, stripeWidth, resultsDir,
				)
			}
		}()
	}
