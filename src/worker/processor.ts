...
function updateConfig(config: Partial<ProcessingConfig>): void {
  ...
  const needsReprocess =
    config.cellSize !== undefined || config.samplingStep !== undefined;
...
self.onmessage = async (event: MessageEvent<WorkerMessage>) => {
  ...
  try {
    switch (type) {
      ...
      case 'pause':
        isPaused = true;
        self.postMessage({ type: 'progress', data: { paused: true } });
        break;

      case 'resume':
        isPaused = false;
        self.postMessage({ type: 'progress', data: { resumed: true } });
        break;
