...
/**
 * Convert rectangles to GeoJSON polygon with holes (for mask rendering)
 * Creates a world-covering polygon with holes where user has explored
 */
export function rectanglesToMaskGeoJSON(
  rectangles: Rectangle[],
  cellSize: number,
  bounds?: { minX: number; minY: number; maxX: number; maxY: number }
): any {
  // World bounds in Web Mercator (approximately)
  ...
  // Each rectangle becomes a hole
  const holes = rectangles.map((rect) => {
    const x1 = rect.minX * cellSize;
    const y1 = rect.minY * cellSize;
    const x2 = (rect.maxX + 1) * cellSize;
    const y2 = (rect.maxY + 1) * cellSize;

    // Holes must be counter-clockwise (opposite of outer ring)
    return [
      [x1, y1],
      [x1, y2],
      [x2, y2],
...
/**
 * Convert rectangles to simple GeoJSON features (for direct rendering)
 */
export function rectanglesToGeoJSON(rectangles: Rectangle[], cellSize: number): any {
  return {
    ...
    features: rectangles.map((rect) => ({
      ...
      geometry: {
        ...
        coordinates: [
          [
            [rect.minX * cellSize, rect.minY * cellSize],
            [(rect.maxX + 1) * cellSize, rect.minY * cellSize],
            [(rect.maxX + 1) * cellSize, (rect.maxY + 1) * cellSize],
            [rect.minX * cellSize, (rect.maxY + 1) * cellSize],
            [rect.minX * cellSize, rect.minY * cellSize],
          ],
        ],
      },
 ...
 */
export function isPointInRectangles(
  x: number,
  y: number,
  rectangles: Rectangle[]
): boolean {
